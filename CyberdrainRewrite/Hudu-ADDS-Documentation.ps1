######################################################################
#                                                                    #
# Wrapper script to run Hudu-Get-ADDS-Documentation in PowerShell 7  #
# Requires values for $HuduAPIKey, $HuduBaseDomain, and $CompanyName #
#                                                                    #
# e.g. Start-Process -FilePath "pwsh.exe" -ArgumentList "-HuduAPIKey #


# Define variable containing PowerShell 7 script
...
if (-not(Test-Path "$env:temp\get-adds.ps1" -PathType leaf)) {
$hudups | out-file "$env:temp\get-adds.ps1"
}
